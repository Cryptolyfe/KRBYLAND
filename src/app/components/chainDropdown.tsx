// src/app/components/chainDropdown.tsx
"use client";
...
export default function ChainDropdown({
  ...
  const selectedChainName =
    chains.find((c) => c.key === selected)?.chainName || "Select Chain";

  return (
    <div className="relative inline-block">
      <button
        onClick={() => setOpen(!open)}
        className="px-2 py-1 text-xs text-white border-2 border-white rounded-full

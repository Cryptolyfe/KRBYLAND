    ...
    <main className="h-screen w-screen bg-black text-white flex flex-col">
      <section className="pt-6 text-center space-y-2">
        <h1 className="text-4xl sm:text-5xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-cyan-400">
          Balanced Scene: Brighter Overheads & Disco Ball
        </h1>
        <h2 className="text-xl sm:text-2xl text-cyan-300">
          Overheads=4 (colored), Floor Spots=0, DiscoBallSpot=6
        </h2>
      </section>
...
function RotatingScene() {
  ...
  return (
    <group ref={sceneRef}>
      {/* Overheads => overheadIntensity=4 => fairly bright, no white */}
      <CeilingLights overheadIntensity={100} />

      {/* No white floor spots => none at all */}
      {/* Or define them colored at a low intensity if you want. */}
      {/* <FloorSpotlight color="blue" floorIntensity={...} /> etc. */}

      {/* Disco ball spotlight => overhead & slightly front => intensity=6 */}
...
/* =========== OverheadLights => purely colored, overheadIntensity=4 =========== */
function CeilingLights({ overheadIntensity=4 }: { overheadIntensity?: number }) {
  const numLights=30;
  // Remove "white"
...
function CeilingLight({
  color, x, z, speed, mode, overheadIntensity=4
